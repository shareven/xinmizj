...
$('#fontSizeAdd').on('click', function () {
	var v = parseInt($fontSize1[0].value);
	if (v < 24) {
		$fontSize1.val(v + 2);
...
$('#fontSizeReduce').on('click', function () {
	var v = parseInt($fontSize1[0].value);
	if (v > 12) {
		$fontSize1.val(v - 2);
		$content.css('font-size', v + 'px');
...
// 行距
$('#lineHeight1').on('change', function () {
	$content.css('line-height', $(this).val() + 'em');
})
// 关闭按钮
...
var $comments = $(".comments .comment-emoji"),
	commentsLength=$(".comments .comment-emoji").length;	
for (var index = 0; index < commentsLength; index++) {
	$comments[index].innerHTML=replace_em($comments[index].innerHTML);
}
...
function replace_em(str) {
	if(str){
		...
		str = str.replace(/\>/g, '&gt;');
		str = str.replace(/\n/g, '<br/>');
		str = str.replace(/\[qq_([0-9]*)\]/g, "<img src='/static/images/emoji/$1.gif' />");
		str = str.replace(/\[em_([0-9]*)\]/g, "<img src='/static/emoji/$1.png'  />");
		return str;
	}
...
function zh_tran(go) {
	...
	go == 't' && ($('.fanti').addClass('active'), $('.jianti').removeClass('active'));
	go == 's' && ($('.jianti').addClass('active'), $('.fanti').removeClass('active'));
	if (go == 'n') {
